...
for(virus in viral_species_include){
  ...
  lasso_coef_summary <- lasso_coefs %>% 
    ...
    ungroup() %>% 
    mutate(key = clean_names[key])
  
  write_rds(lasso_coef_summary, h("lassos", paste0(report_country, "_", virus, ".rds")))
}
```

```{r plots}

walk(c( "Murine coronavirus", "PREDICT_CoV-17",  "PREDICT_CoV-82"), function(virus){
  
  lasso_coef_summary <- read_rds(h("lassos", paste0(report_country, "_", virus, ".rds")))
  
  coefs <- lasso_coef_summary %>% 
    filter(support > 0.5) %>% 
    mutate(key = paste0(key, " (s = ", round(support, 2), ")")) %>% 
    arrange(desc(median_coef)) %>%
    mutate(key = fct_reorder(key, median_coef)) %>%
    mutate(positive = median_coef > 0)

  ggplot(coefs, aes(x = key, y = median_coef, fill=positive)) +
    ...
    coord_flip() +
    labs(x = "", y = "Odds Ratios", title = str_replace(virus, "_", " ")) +
    theme_minimal() +
