...
for(vir in viral.species.detected.in.country) {
  ...
  species_dat <- species_dat %>%
    mutate(color = ifelse(n_animals_w_detections>0,
                          unname(pal_spec[species_scientific_name]),
                          "#ffffff"))  %>%
    mutate(lab = ifelse(n_animals_w_detections>0, 
                        paste0("PREDICT ", species_scientific_name, " (", n_animals_tested, " animals tested, ", n_animals_w_detections, " animals positive)"),
                        paste0("PREDICT ", species_scientific_name, " non-detect (", n_animals_tested, " animals tested)")))
