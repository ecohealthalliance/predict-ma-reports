...
if(!file.exists(h("map-layers/gpw-v4-population-density_2015.tif"))){
  download.file("https://s3.amazonaws.com/country-maps/gpw-v4-population-density_2015.tif", h("map-layers/gpw-v4-population-density_2015.tif"))
}
...
# mammal livestock density
if(!file.exists(h("map-layers/summed_mammal_livestock.tif"))){
  download.file("https://s3.amazonaws.com/country-maps/mammal_livestock/summed_mammal_livestock.tif", h("map-layers/summed_mammal_livestock.tif"))
}

mammal_dens <- raster(h("map-layers/summed_mammal_livestock.tif")) %>%
  crop(., admin) %>%
  mask(., admin)

values(mammal_dens)[values(mammal_dens)==0] <- 1
values(mammal_dens) <- log10(values(mammal_dens))

```

```{r human-dens-map}
min_val <- floor(minValue(human_dens))
max_val <- ceiling(maxValue(human_dens))
pal <- colorBin(palette = c("azure4", "yellow", "coral2"), domain = -1:max_val, na.color = "transparent")
