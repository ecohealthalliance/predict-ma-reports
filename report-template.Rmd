...
full.association.table <- hp3.associations %>%
  ...
  arrange(viral_species, species_scientific_name) %>%
...
<details>
<summary>Expansion of Known Viral Host Range</summary>

```{r viral-host-range, fig.align = "center", fig.height = 5, fig.width = 10}
...
Viral host species ranges are shown over country risk maps below. Use the bottom-left icon to control the map display.  The listed viruses have been detected under PREDICT surveillance in `r params$country`.  Clicking on a virus displays the International Union for Conservation of Nature (IUCN) global ranges for the host species in which the virus was detected (hover over polygons to see host species' names). Scroll to the bottom of the list to toggle between the human population density layer, as estimated by the Center for International Earth Science Information Network (CIESIN), and the mammalian livestock density layer, based on a model developed by Robinson et al. (2014).  These density layers are shown only for `r params$country`.  Other EcoHealth Alliance PREDICT countries are outlined in white.
...
# Viral-species pairs
animal_viral_pairs <- d2 %>% 
  drop_na(viral_species) %>%
  group_by(species_scientific_name, viral_species, site_latitude, site_longitude) %>%
  summarize(n_detects = as.character(n())) %>%
  ungroup() %>%
  mutate(
    species_scientific_name = case_when(
      str_detect(species_scientific_name, " cf. ") ~ 
...
for(vir in unique(animal_viral_pairs$viral_species)){
  ...
  species_detects <- animal_viral_pairs %>%
    ...
    mutate(color = factor(species_scientific_name, labels = suppressWarnings(brewer.pal(n_distinct(species_scientific_name), "Set3"))[1:n_distinct(species_scientific_name)]))
  
  if(n_distinct(species_detects$species_scientific_name) < 11){
    
    for(spec in unique(species_detects$species_scientific_name)){
      
      iucn_sub <- iucn_detects %>% filter(binomial==spec)
      species_detects_sub <- species_detects %>% filter(species_scientific_name == spec)
      
      lf <- lf %>%
        addPolygons(data = iucn_sub,
                    stroke = TRUE, color = unique(species_detects_sub$color), weight = 2, opacity = opac,
                    fill = TRUE, fillColor = unique(species_detects_sub$color), fillOpacity = opac,
                    label = spec,
                    group = vir) 
    }
    
    lf <- lf %>%
              addCircleMarkers(data = species_detects, radius = 3,
                         lng = ~site_longitude, lat = ~site_latitude, 
                         stroke = FALSE, 
                         fill = TRUE, fillColor = species_detects$color, fillOpacity = 1,
                         label = ~paste("PREDICT ", species_scientific_name, "(n detect = ", n_detects, ")"),
                         group = vir)
    
    # iucn_sub <- iucn_detects %>% filter(binomial %in% species)
    # test <- st_intersection(iucn_sub)
    ...
    lf <- lf %>%
      addPolygons(data = iucn_sub,
                  ...
                  group = vir) %>%
              addCircleMarkers(data = species_detects_sub, radius = 3,
                         lng = ~site_longitude, lat = ~site_latitude, 
                         stroke = FALSE, 
                         fill = TRUE, fillColor = brewer.pal(3, "Accent")[1], fillOpacity = 1,
                         label = ~paste("PREDICT ", species_scientific_name, "(n detect = ", n_detects, ")"),
                         group = vir)
...
Robinson, T.P., Wint, G.W., Conchedda, G., Van Boeckel, T.P., Ercoli, V., Palamara, E., Cinardi, G., D’Aietti, L., Hay, S.I. and Gilbert, M., 2014. Mapping the global distribution of livestock. PloS one, 9(5), p.e96084.
