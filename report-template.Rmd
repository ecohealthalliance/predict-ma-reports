...
1) Human and livestock population density in `r params$country`.  

2) Ranges of mammalian host species in which viruses have been detected in `r params$country` under PREDICT surveillance.

To control the layers visable on the map, click the bottom left icon.  

-   This will bring up a list of viruses that have been detected in mammals in `r params$country`.  Click on a virus to see global ranges from the International Union for Conservation of Nature (IUCN) for the host species.  Hover over polygons to see the host species names. Points on the map show where the host species was sampled in `r params$country`.  Solid color circles indicate that the virus was detected in one or more specimen; circles filled with white indicate non-detects.  Hover over points to see counts of animals tested.
