...
animal.grouping.vars <-
  c("country", "concurrent_sampling_site", "site_name", "district", "disease_transmission_interfaces", "human_density_impact", "class", "taxa_group_mod", "scientific_name", "test_requested", "viral_species", "human_threat", "specimen_type")

animal_virus_summary <- d2_country %>%
  filter(!is.na(viral_species)) %>%
  group_by_at(vars(animal.grouping.vars)) %>% 
  summarize(n_positives = n_distinct(specimen_id)) %>% 
  ungroup() %>% 
  arrange(country, site_name, taxa_group_mod, 
          scientific_name, viral_species, n_positives)

threat.interpretation <- d2_country %>%
  filter(
    !is.na(viral_species),
    human_threat == "yes"
  ) %>%
  ...
  dplyr::select(-interpretation_length)

text.color <- "#787272"

animal_virus_summary_table <- animal_virus_summary %>%
  mutate(site_name = paste0(site_name, " (", district, ")")) %>% 
  group_by(test_requested, viral_species, human_threat) %>%
  summarize(
    ...
    specimen_type = paste(unique(specimen_type), collapse = "\n"),
  ) %>%
  ungroup() %>% 
  left_join(country_virus_lookup, 
            by = "viral_species") %>%
  left_join(., threat.interpretation, by = "viral_species") %>%
  ...
  mutate(
    ...
    human_threat = ifelse(human_threat == "yes", "yes\n(scroll over for details)", human_threat)
  ) %>%
  arrange(test_requested, viral_species) %>%
  rename("Other PREDICT Detections\n(Gov. Approved)" = `Concurrent Detections`,
         "Viral Test Type" = test_requested,
         "Virus Name" = viral_species,
         "Known Human Threat?" = human_threat,
         "Specimen" = specimen_type,
         "Number of Detections" = n_detections,
         "Taxa Groups" = taxa_groups,
         "Species Names" = species,
         "Site Names" = site_names) 

write_csv(animal_virus_summary_table, h(paste0("data/animal_viral_pairs/", report_country, "_animal_virus_summary_table.csv")))

animal_virus_summary_table %>% 
  mutate_at(
    vars(-interpretation),
...
host.table <- animal_virus_summary %>%
  group_by(scientific_name, taxa_group_mod) %>%
  summarize(
...
hp3.associations <- read_csv(h("data", "associations.csv")) %>%
  ...
  mutate(
    ...
    scientific_name = str_replace_all(host_name, "_", " "),
...
p2.associations <- read_csv(h("data", "P2_virus_mammal_host_associations.csv")) %>%
  ...
  group_by(viral_species, scientific_name) %>%
...
full.association.table <- hp3.associations %>%
  dplyr::select(viral_species, scientific_name, project) %>%
  filter(viral_species %in% viruses.in.country) %>%
  bind_rows(., p2.associations) %>%
  arrange(viral_species, scientific_name, desc(project)) %>%
  distinct(viral_species, scientific_name, .keep_all = TRUE) %>%
  mutate(
...
country_centroid <- CoordinateCleaner::countryref %>% 
  filter(type == "country",
         iso3 == countrycode(sourcevar = country_mod,
                             origin = "country.name",
                             destination = "iso3c")) %>%
  slice(1)
...
animal_viral_pairs <- read_csv(h("data", "viral_species_testing_by_host_and_site.csv")) %>%
  filter(country == !!report_country
        # scientific_name %in% unique(iucn$binomial) # only keep host species that have ranges
  )
...
write_csv(animal_viral_pairs, h(paste0("data/animal_viral_pairs/", report_country, "_animal_viral_pairs.csv")))

animal_viral_pairs <- animal_viral_pairs %>% 
  filter(scientific_name %in% unique(iucn$binomial)) # only keep host species that have ranges
```
...
# Add viruses to leaflet
for(vir in viral.species.detected.in.country) {
  
  # subset detect and non-detect data
  species_dat <- animal_viral_pairs %>%
    filter(viral_species == vir) %>%
    arrange(n_animals_w_detections)
  
  # get unique species hosts for given virus
  species <- species_dat %>% 
    filter(n_animals_w_detections>0) %>% 
  ...
  # set palette 
  ifelse(length(species) < 11,
         pal_spec <- suppressWarnings(brewer.pal(length(species), "Set3")),
         pal_spec <- rep("#7FC97F", length(species))
  )
  names(pal_spec) <- species
  
  # add color and labels to species data
  species_dat <- species_dat %>%
    mutate(color = ifelse(n_animals_w_detections>0,
                          unname(pal_spec[scientific_name]),
                          "#ffffff"))  %>%
    mutate(lab = ifelse(n_animals_w_detections>0, 
                        paste0("PREDICT ", scientific_name, " (", n_animals_tested, " animals tested, ", n_animals_w_detections, " animals positive)"),
                        paste0("PREDICT ", scientific_name, " non-detect (", n_animals_tested, " animals tested)")))
  ...
  # add points
  lf <- lf %>%
    addCircleMarkers(data = species_dat, radius = rad,
                     lng = ~jitter(event_longitude), lat = ~jitter(event_latitude), 
                     stroke = TRUE, color = ~color, weight = 1.5, opacity = 1,
...
mz <- read_csv(h("data", "hp3_zoo_predictions.csv")) %>%
  ...
  mutate(
    ...
    filter(relative_zoonoses == "Top 10% of all mammals")
...
species.sampled.in.country <- d2_country %>%
...
mz.subset <- left_join(mz.subset, specimen.count, by = c("species" = "scientific_name"))
write_csv(mz.subset, h("data", "viral_prioritization", paste0(report_country, "_viral_prioritization.csv")))
```
