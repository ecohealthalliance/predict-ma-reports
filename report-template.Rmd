...
animal_viral_pairs <- d2 %>% 
  ...
  count() %>%  
  ungroup() %>%
  mutate(n_detect = ifelse(is.na(viral_species), "0", as.character(n))) %>%
  mutate(
    species_scientific_name = case_when(
      str_detect(species_scientific_name, " cf. ") ~ 
        str_replace(species_scientific_name, " cf. ", " "),
...
for(vir in unique(animal_viral_pairs$viral_species)){
  ...
  species_detects <- animal_viral_pairs %>%
    filter(viral_species == vir, species_scientific_name %in% unique(iucn_detects$binomial)) 
  species <- unique(species_detects$species_scientific_name)
  
  species_non_detects <- animal_viral_pairs %>%
    filter(species_scientific_name %in% species_detects$species_scientific_name, is.na(viral_species))
  ...
  if(length(species) < 11){
    ...
    lf <- lf %>%
      addCircleMarkers(data = species_non_detects, radius = rad,
                       ...
                       stroke = TRUE, color = ~unname(pal_spec[species_scientific_name]), weight = 1, opacity = 1,
                       fill = TRUE, fillColor = "#ffffff", fillOpacity = 1,
                       label = ~paste("PREDICT ", species_scientific_name, "non-detect"),
                       group = vir) %>%
            addCircleMarkers(data = species_detects, radius = rad, 
                       ...
                       label = ~paste("PREDICT ", species_scientific_name, "(n detect = ", n_detect, ")"),
                       group = vir) 
    ...
    lf <- lf %>%
      addPolygons(data = iucn_sub,
          ...
          addCircleMarkers(data = species_non_detects, radius = rad,
                       lng = ~jitter(site_longitude), lat = ~jitter(site_latitude), 
                       stroke = TRUE, color =  "#7FC97F", weight = 1, opacity = 1,
                       fill = TRUE, fillColor = "#ffffff", fillOpacity = 1,
                       label = ~paste("PREDICT ", species_scientific_name, "non-detect"),
                       group = vir) %>%
            addCircleMarkers(data = species_detects, radius = rad, 
